...
def link_contents(project_rpath):
    ...
    def extract_includes(rpath, name, is_header = True):
        ...
        regex = '.*".*".*'
        user_includes = {incl.split('"')[1][:-2] for incl in cppHeader.includes 
        if not re.search(regex, incl) is None and not "build_info.h" in incl}
        ...
        return (user_includes, extra_includes)
    ...
    def check_circular_dependency(collector):
        ...
        def check(rpath, name, is_header, access):
            ...
            if dep in dependencies:
                raise Exception("Error: There is a header-only dependency cycle in the project. Please resolve it.")
            elif is_header: dependencies.add(dep)
    ...
    def collect_dependecies(rpath, name, is_header, access):
        ...
        includes, extra_includes = extract_includes(rpath, name, is_header)
